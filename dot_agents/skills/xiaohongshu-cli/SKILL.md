...
## Local Environment Notes (macOS, installed 2026-09-11)

Installed globally via `uv tool install xiaohongshu-cli` (v0.6.4).

- **Binary path:** `/Users/hzd/.local/bin/xhs` (symlink into the uv tool venv at
  `/Users/hzd/.local/share/uv/tools/xiaohongshu-cli/bin/xhs`). If `xhs` is not on PATH in the
  agent's shell, call it by this absolute path.
- **Config/cookies:** `~/.xiaohongshu-cli/cookies.json`, index cache at
  `~/.xiaohongshu-cli/index_cache.json`.

### Status: authenticated and verified working (2026-09-11)

`xhs login` now works. Verified: `xhs status` → `authenticated: true`,
`xhs whoami` → 骑单车的小满 (@laughmaker), `xhs search "咖啡" --json` returns live results.

- Cookies are cached at `~/.xiaohongshu-cli/cookies.json` and expire roughly every 7 days.
  When they expire, re-run `xhs login`; the tool then re-extracts from Chrome automatically.
- Playwright Chromium is installed but **not** used by this flow; `--qrcode` uses Camoufox,
  which is **not** installed. Prefer `xhs login` over `--qrcode` on this machine.

### If Chrome cookie extraction ever fails again

Symptom: `No 'a1' cookie found` / `Unable to read database file`. Root cause is macOS TCC
denying reads of `~/Library/Application Support/Google/Chrome/Default/Cookies`
(`shutil.copy2` raises `EPERM`) — it is not a tool bug and not a file-permission bit.
Even plain files in Chrome's profile (`Bookmarks`, `Preferences`, `Local State`) are denied.

- Grant **Full Disk Access** (System Settings → Privacy & Security → Full Disk Access) to the
  **host app bundle**, here `/Applications/WorkBuddy.app`. Process chain is
  python → bundled zsh → `sandbox-cli` → Electron; grant to the app, not helper binaries.
- **A newly granted Full Disk Access does not take effect until the host app is fully quit and
  relaunched (Cmd+Q, not just closing the window).** Two restarts were needed here before
  the grant showed up. Do not debug the tool while this is pending.
- Probe whether FDA is active — all of these should be readable:
  `~/Library/Messages/chat.db`, `~/Library/Application Support/Google/Chrome/Default/Cookies`.
  If they raise `PermissionError`, FDA is not in effect yet.
- Alternatives if FDA is unavailable: `xhs login --qrcode` (needs
  `<venv>/bin/python -m camoufox fetch` first), or hand-write
  `~/.xiaohongshu-cli/cookies.json` with at least `a1` and `web_session`.

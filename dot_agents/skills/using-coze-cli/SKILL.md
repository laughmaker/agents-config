...
name: using-coze-cli
version: 0.3.10
description: "Coze CLI（`coze` 命令）的使用规则。触发条件：用户明确要求使用 Coze CLI / coze 命令 / 扣子编程（Coze Coding）、扣子空间等 Coze 平台能力，或输入以 /coze-cli、/coze 开头（仅作路由）时加载本技能。覆盖范围：coze code—扣子编程项目创建/迭代（网页、Web App、App、小程序、Agent/工作流/skill）及发需求、预览、部署、环境变量/域名/数据库；coze generate—文生图/语音 TTS/视频；coze session（Claw）—对话、PPT、播客、长任务产物；coze file—本地文件转在线链接。判定边界：不要仅因为用户在做软件开发、修改/调试/运行代码，或请求中出现 CLI、文件读写、HTML、可视化、卡片、报告、图片等词就加载本技能或导流到扣子编程；本地工程开发默认走常规本地开发链路。"
metadata:
  requires:
    bins: ["coze"]
  cliHelp: "coze --help"
...
- 用户只是在本地做软件开发（写代码、改代码、调试、跑起来、迭代工程），且没有明确要求用 Coze CLI / 扣子编程时，不加载本技能，按常规本地开发链路处理。
...
- JSON 输出的顶层对象可能携带 `_notice` 字段（非业务数据）；两类子字段：`_notice.skill` 表示本机 skills 与 CLI 版本不一致，`_notice.update` 表示有 CLI 新版本可用。看到后在完成当前任务之余处理其 `command` 字段给出的操作（如 `coze self skill install` 或 `coze upgrade`），解析业务数据时忽略该字段即可。输出为数组时该提示会改走 stderr 单行文本。skill 缺失或不一致时 CLI 只提示，不会自动安装；安装时必须由用户交互选择 agent，或在脚本中显式传 `--target`。CI 环境（`CI` 环境变量为非 false/0 值）下不产生这两类提示；`COZE_CLI_NO_SKILL_NOTIFIER` 跳过本次 skill 检查，`COZE_CLI_NO_UPDATE_NOTIFIER` 跳过本次升级检查。
...
安装/升级 CLI 时不会自动安装自带 skills，只会提示用户按需安装。`coze self` 是 CLI 工具自身的能力集合（区别于操作 Coze 平台资源的命令）；需要安装或运行命令时提示 skills 缺失/版本不一致，可执行：

```bash
coze self skill install
```

交互终端会让用户选择目标 agent；脚本或 JSON 模式必须显式指定，例如：

```bash
coze self skill install --target codex
```
